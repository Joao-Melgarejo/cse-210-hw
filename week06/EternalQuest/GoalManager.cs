    ...
    public void LoadGoals()
    {
        ...
        if (File.Exists(fileName))
        {
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                if (type == "SimpleGoal")
                {
                    var g = new SimpleGoal(data[0], data[1], data[2]);
                ...
                {
                    var g = new ChecklistGoal(data[0], data[1], data[2], int.Parse(data[4]), int.Parse(data[3]));
                    int amountCompleted = int.Parse(data[5]);
                    for(int j=0; j < amountCompleted; j++) g.RecordEvent(); 
                    _goals.Add(g);
                }
            }
        }
